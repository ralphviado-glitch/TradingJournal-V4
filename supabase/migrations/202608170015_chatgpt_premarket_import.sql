-- Additive V1.2.1 correction for the already-deployed 014 migration.
-- Adds manual ChatGPT import metadata and replaces the approval RPC so omitted
-- premarket levels preserve existing user-entered values.
alter table public.market_days
  add column if not exists premarket_data_included boolean not null default false;
...
begin
  ...
  v_generation_source := case
    when p_metadata->>'generation_source' in ('ai_assisted', 'chatgpt_manual_import') then p_metadata->>'generation_source'
    else 'ai_assisted'
  end;
  ...
  insert into public.market_days (
    ...
    generation_source, ai_generated_at, ai_model, ai_data_as_of, ai_prompt_version, premarket_data_included, updated_at
  ) values (
    ...
    v_generation_source, nullif(p_metadata->>'ai_generated_at','')::timestamptz, nullif(p_metadata->>'ai_model',''), nullif(p_metadata->>'ai_data_as_of','')::timestamptz, p_metadata->>'ai_prompt_version', false, now()
    ...
    spy_pdh=excluded.spy_pdh, spy_pdl=excluded.spy_pdl, spy_pmh=coalesce(excluded.spy_pmh, market_days.spy_pmh), spy_pml=coalesce(excluded.spy_pml, market_days.spy_pml), spy_liquidity_target=excluded.spy_liquidity_target, spy_most_important_level=excluded.spy_most_important_level, spy_bull_trigger=excluded.spy_bull_trigger, spy_bear_trigger=excluded.spy_bear_trigger, spy_game_plan=excluded.spy_game_plan,
    generation_source=excluded.generation_source, ai_generated_at=excluded.ai_generated_at, ai_model=excluded.ai_model, ai_data_as_of=excluded.ai_data_as_of, ai_prompt_version=excluded.ai_prompt_version, premarket_data_included=false, updated_at=now();
