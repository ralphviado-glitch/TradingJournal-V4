...
const watchlistNames = ["ticker", "rating", "weeklyBias", "intradayBias", "relativeStrength", "preferredDirection", "confidence", "longPlan", "longTrigger", "longTarget", "longInvalidation", "shortPlan", "shortTrigger", "shortTarget", "shortInvalidation", "bottomLine"];
const watchlistProperties = { ...textFields(watchlistNames), dataAvailable: { type: "boolean" }, longScenarioEnabled: { type: "boolean" }, shortScenarioEnabled: { type: "boolean" } };
...
const systemPrompt = `You are a professional proprietary-trading preparation assistant creating an initial structural plan from higher-timeframe and prior regular-session data only. Create conditional execution scenarios, never predictions or orders. Analyze QQQ and SPY first, then every requested stock. Prioritize structure, trend, liquidity, recent regular-session relative strength/weakness, expansion potential, risk/reward, displacement, break-and-retest quality, and QQQ/SPY correlation. Use IF price does X, THEN consider Y language without certainty. Every enabled bullish and bearish thesis needs an invalidation. Both scenarios may be enabled. Keep each stock field concise and live-trading scannable. Confidence must be Low, Medium, or High. Preferred direction must be Long, Short, Both, or Neutral. Rating should use a coarse A/B/C format. Never invent news/events or numeric levels. Do not mention or infer PMH, PML, premarket strength, gaps, overnight structure, extended-hours volume, or live premarket relative strength. Prefer PDH, PDL, daily support/resistance, and recent swing references. Phrase relative strength as recent or regular-session relative strength. For dataAvailable=false return dataAvailable=false, disable both scenarios, and say Data unavailable in the bottom line.`;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    draft.watchlist = draft.watchlist.map((item: Record<string, unknown>) => ({ ...item, relativeStrength: item.dataAvailable === false ? item.relativeStrength : `Recent regular-session: ${String(item.relativeStrength || "Neutral vs QQQ/SPY")}`, levels: { pdh: byTicker[item.ticker as string]?.previousDayHigh ?? null, pdl: byTicker[item.ticker as string]?.previousDayLow ?? null, pmh: null, pml: null, atr: byTicker[item.ticker as string]?.atr ?? null } }));
