      ...
      <header className="app-header">
        ...
        {importSummary ? (
          <section className="import-processing-summary" aria-label="Import processing summary">
            ...
            {importSummary.filledRows != null ? <><p>Trade The Pool Account: {importSummary.canonicalAccount} · Selection method: {importSummary.selectionMethod}</p><p>Filled rows: {importSummary.filledRows} · Duplicate fills excluded: {importSummary.duplicateFilledRows || 0} · Accounts detected: {importSummary.accountCount}</p><p>Completed trades: {importSummary.completedTrades} · Open positions: {importSummary.incompletePositions}</p><p>Gross P&amp;L: ${Number(importSummary.grossPnl).toFixed(2)} · Fees: ${Number(importSummary.totalFees).toFixed(2)} · Net P&amp;L: ${Number(importSummary.netPnl).toFixed(2)}</p><p>Fee source: {importSummary.feeSource} · Basis: {importSummary.pnlBasis} · Status: {importSummary.status}</p></> : null}
